"""LLM provider management and generation functions."""
...
from config.models import PROVIDERS, SYSTEM_PROMPTS
...
class LLMProviderNotAvailable(RuntimeError):
    """Raised when an LLM backend cannot be initialized."""
...
def get_llm_model(selected_provider: ProviderKey = "OLLAMA"):
    ...
    Args:
        selected_provider: The provider to use ("OLLAMA" or "GROQ")
    ...
    Raises:
        ...
        LLMProviderNotAvailable: If provider cannot be initialized
    """
...
def generate_tutor_intro(
    ...
    """
    Generate an introductory message from the tutor.

    Args:
        topic: The topic or learner goal
        context: Optional reference material
        provider: LLM provider to use

    Returns:
        Generated tutor introduction text
    """
...
def generate_tutor_response(
    ...
    """
    Generate a tutor response based on learner input.

    Args:
        learner_message: The learner's message
